...
unsigned char xx[] = { 2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 38,
41, 44, 47, 50, 53, 56, 59, 62, 65, 68, 71, 74, 77, 80 }; //27 values for x
unsigned char yy[] = { 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42 }; // 14 values for y
...
unsigned char snake_cnt;

static void (*start_screen_callback)(unsigned char usr_cnt);
static void (*end_screen_callback)(unsigned char usr_cnt);
...
void registet_end_screen_callback( void (*callback)(unsigned char usr_cnt))
{
	end_screen_callback = callback;
}


void snake_init(unsigned char cnt)
{
